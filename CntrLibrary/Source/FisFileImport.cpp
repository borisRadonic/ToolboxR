...
FuzzyController* FisFileImport::toFuzzyController()
{
	...
	for (auto& rule : _rules)
	{
		...
		std::vector<std::double_t> thenCoefsC; //empty for Mamdani controller !!! TODO?????
						
		pController->addRule(bo, "rule" + std::to_string(rc) , "", inVariables, vecHedgesVec, linVariables, outVariable, outLingVariable, thenCoefsC, 0.0, rule.weight);
